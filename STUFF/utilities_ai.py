...
def parse_document(document_ID):
    logging.info(f'Document parsing started with Pubmed ID {document_ID}')
...
def pubtator(document_ID):
    logging.info(f'Pubtator search started with Pubmed ID {document_ID}')
    FORMAT = 'biocxml'
    TYPE = "pmids"
    ...
    logging.info("Waiting 1.5 secs for Pubtator")
    time.sleep(1.5)
    ...
    doc = root[3]
    passage = doc[1:]
    TEXT, ELEMENT, IDENTIFIER, TYPE = ([] for _ in range(4))
    for i in passage:
